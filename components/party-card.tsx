...
import { Edit, Lock, Shield, Sword, Heart, Trash2, ChevronUp, ChevronDown } from "lucide-react"
    ...
    >
      <div ref={headerRef} className={`party-header ${isAuthenticated ? "cursor-default" : "cursor-default"}`}>
        <div className="party-name">
          ...
          {party.previousOrder !== undefined && party.previousOrder !== party.order && (
            <span className="text-xs ml-2 opacity-70">(Moved from {party.previousOrder})</span>
          )}
        </div>
        <div className="flex items-center">
          ...
          <button className="text-blue-100 hover:text-white mr-2" onClick={handleRenameClick}>
            <Edit className="h-4 w-4" />
          </button>
          <button className="text-red-400 hover:text-red-300" onClick={handleRemoveParty}>
...
interface MemberInPartyProps {
  member: Member
  onRemove: (memberId: string, partyId: string | null) => void
  isAuthenticated: boolean
}

function MemberInParty({ member, onRemove, isAuthenticated }: MemberInPartyProps) {
  ...
  return (
    ...
    >
      ...
      {isAuthenticated && (
        <button className="text-gray-400 hover:text-white" onClick={() => onRemove(member.id, null)}>
          ×
        </button>
      )}
